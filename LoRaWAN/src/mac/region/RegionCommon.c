...
TimerTime_t RegionCommonUpdateBandTimeOff( bool joined, bool dutyCycle, Band_t* bands, uint8_t nbBands )
{
    ...
    for( uint8_t i = 0; i < nbBands; i++ )
    {
        if( joined == false )
        {
            ...
            MSG_DBG("txDoneTime:%d\r\n",txDoneTime);
            MSG_DBG("TimerGetElapsedTime( bands[i].LastJoinTxDoneTime ):%d\r\n",TimerGetElapsedTime( bands[i].LastJoinTxDoneTime ));
            MSG_DBG("TimerGetElapsedTime( bands[i].LastTxDoneTime ):%d\r\n",TimerGetElapsedTime( bands[i].LastTxDoneTime ));
            MSG_DBG("bands[i].TimeOff:%d\r\n",bands[i].TimeOff);
    ...
    MSG_DBG("bands[i].TimeOff:%d exist \r\n",bands[0].TimeOff);    
    return nextTxDelay;
}
